...
NAN_METHOD( InteropType::New )
{
	...
	info.This()->DefineOwnProperty(
		...
		asyncCtorLocal->NewInstance( Nan::GetCurrentContext(),1, argv ).ToLocalChecked(),
...
void InteropType::InvokeSyncOrAsync( bool isAsync, Nan::NAN_METHOD_ARGS_TYPE info )
{
	...
	else
	{
		...
		// Create the promise.
		v8::Isolate *isolateR = info.GetIsolate();
		auto resolver = v8::Promise::Resolver::New( isolateR->GetCurrentContext() );
...
NAN_INDEX_GETTER( InteropType::GetIndex )
{
	...
	info.GetReturnValue().Set( getter->Call(Nan::GetCurrentContext(), info.This(), 1, &localIndex ).ToLocalChecked() );
}
...
namespace
{
	...
	void DoInvokeAfter( uv_work_t* req, int status )
	{
		...
		try
		{
			...
			resolver->Resolve(Nan::GetCurrentContext(), baton->methodInfo->GetInvokeResult( baton->hr, baton->result, baton->exception ) );
		}
		catch( JsException ex )
		{
			// Reject the promise on exception.
			resolver->Reject(Nan::GetCurrentContext(), ex.GetError() );
		}
	}
