...
void JsException::ThrowParameter( int i, JsException& ex )
{
	...
	v8::String::Utf8Value errStr( v8::Isolate::GetCurrent(),errMsg->ToString(Nan::GetCurrentContext()).ToLocalChecked() );
...
void JsException::ThrowMismatch( const CComPtr< ITypeInfo >& expected, v8::Local< v8::Value > actual )
{
	CComBSTR bstrName;
	expected->GetDocumentation( MEMBERID_NIL, &bstrName, nullptr, nullptr, nullptr );
	v8::String::Utf8Value actualStr(v8::Isolate::GetCurrent(),actual->ToString(Nan::GetCurrentContext()).ToLocalChecked() );
